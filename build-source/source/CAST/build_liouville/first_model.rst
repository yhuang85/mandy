...
Since the contact form grows in the direction of :math:`X = \p_s`, it's necessary that the contact form on :math:`Y_1` is "greater than" that on :math:`Y_0`, at least over the overlap. In other words, if :math:`\alpha` is the (standard) contact form on :math:`\RR^{2n-1}`, then :math:`\phi^\ast (\alpha) = g \alpha` where :math:`g \geq 1`.
